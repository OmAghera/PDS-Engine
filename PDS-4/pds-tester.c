...
static struct RelationInfo *find_relation(const char *rel_name){
    char rel_file[120];
    snprintf(rel_file, sizeof(rel_file), "%s.rel", rel_name);
    for(int idx = 0; idx < db_info.rel_count; idx++){
        if(strcmp(db_info.rInfo[idx].rel_name, rel_file) == 0){
...
int pds_create(char* dbname) { return create_db(dbname); }
int pds_open(char* dbname) { return open_db(dbname); }
int pds_create_table(char* tname, int rec_size) { return create_table(tname, rec_size); }
...
void process_line( char *test_case ) {
    ...
    if( !strcmp(command,"CREATE") ){
        ...
        sscanf(test_case, "%s %s %s", command, tname1, param1);
        if( !strcmp(param1,"0") ) expected_status = SUCCESS; else expected_status = FAILURE;
        status = pds_open(tname1);
        if( status == expected_status ) TREPORT("PASS", "");
        else { sprintf(info,"pds_open returned status %d",status); TREPORT("FAIL", info); }
    }
    else if( !strcmp(command,"CREATE_TABLE") ){
        sscanf(test_case, "%s %s %s", command, tname1, param1);
        if( !strcmp(param1,"0") ) expected_status = SUCCESS; else expected_status = FAILURE;
        status = pds_create_table(tname1, rec_size);
        if( status == expected_status ) TREPORT("PASS", "");
        else { sprintf(info,"create_table returned status %d",status); TREPORT("FAIL", info); }
    }
    ...
    // --- NEW RELATIONAL COMMANDS ---
        ...
        if( !strcmp(param1,"0") ) expected_status = SUCCESS; else expected_status = FAILURE;
        struct RelationInfo *rel = find_relation(rel_name);
        status = (rel == NULL) ? FAILURE : open_relation(rel);
        if( status == expected_status ) TREPORT("PASS", "");
        ...
        sscanf(test_case, "%s %s %s %s %s", command, rel_name, pk_str, rk_str, param1);
        if( !strcmp(param1,"0") ) expected_status = SUCCESS; else expected_status = FAILURE;
        sscanf(pk_str, "%d", &id1);
        sscanf(rk_str, "%d", &id2);
        status = store_relation(rel_name, id1, id2);
        if( status == expected_status ) TREPORT("PASS", "");
        else { sprintf(info,"store_relation returned status %d",status); TREPORT("FAIL", info); }
    }
    else if( !strcmp(command,"SEARCH_REL") ){
        sscanf(test_case, "%s %s %s %s", command, rel_name, param1, param2);
        char expected_name[100], expected_address[200], expected_email[50];
        if( !strcmp(param2,"0") ){
            sscanf(test_case, "%*s %*s %*s %*s %s %s %s", expected_name, expected_address, expected_email);
            expected_status = SUCCESS;
        ...
        sscanf(param1, "%d", &id1);
        status = get_relation(rel_name, &testHospital, id1);
        if( status != expected_status ){
        ...
        sscanf(test_case, "%s %s %s %s %s", command, rel_name, param1, tname1, param2);
        if( !strcmp(param2,"0") ) expected_status = SUCCESS; else expected_status = FAILURE;
        sscanf(param1, "%d", &id1);
        status = delete_relation(rel_name, id1, tname1);
        if( status == expected_status ) TREPORT("PASS", "");
        ...
        if( !strcmp(param1,"0") ) expected_status = SUCCESS; else expected_status = FAILURE;
        struct RelationInfo *rel = find_relation(rel_name);
        status = (rel == NULL) ? FAILURE : close_relation(rel);
        if( status == expected_status ) TREPORT("PASS", "");
