...
struct DBInfo{
    struct TableInfo tinfo[100];
    struct RelationInfo rInfo[100];
    char dbname[100];
    int num_table;
    int rel_count;
    int db_status;
...
int open_db(char *dbname); //fopen in rb+
...
int get_relation(char*rel_name,void * related_rec,int search_rel_key);
int delete_relation(char* rel_name,int key,char * table_name);
